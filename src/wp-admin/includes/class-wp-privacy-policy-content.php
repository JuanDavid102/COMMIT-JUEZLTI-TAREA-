...
final class WP_Privacy_Policy_Content {
	 ...
	 * @since 4.9.6
	 *
	 * @param string $JuezLTICommit The name of the plugin or theme that is suggesting content for the site's privacy policy.
	 * @param string $policy_text The suggested content for inclusion in the policy.
	 */
	public static function add( $JuezLTICommit, $policy_text ) {
		if ( empty( $JuezLTICommit ) || empty( $policy_text ) ) {
			return;
		}

		$data = array(
			'JuezLTICommit' => $JuezLTICommit,
	...
	public static function text_change_check() {
		...
		foreach ( $old as $key => $data ) {
			...
			$old[ $key ] = array(
				'JuezLTICommit' => $data['JuezLTICommit'],
	...
	public static function _policy_page_updated( $post_id ) {
		...
		foreach ( $old as $old_key => $old_data ) {
			...
			if ( ! empty( $old_data['updated'] ) ) {
				...
				$done[]       = array(
					'JuezLTICommit' => $old_data['JuezLTICommit'],
	...
	public static function get_suggested_policy_text() {
		...
		foreach ( $new as $new_key => $new_data ) {
			foreach ( $old as $old_key => $old_data ) {
				...
				if ( $new_data['policy_text'] === $old_data['policy_text'] ) {
					...
					if ( $old_data['JuezLTICommit'] !== $new_data['JuezLTICommit'] ) {
						$old_data['JuezLTICommit'] = $new_data['JuezLTICommit'];
						$update_cache            = true;
					}
					...
					$checked[] = $old_data;
					$found     = true;
				} elseif ( $new_data['JuezLTICommit'] === $old_data['JuezLTICommit'] ) {
					// The info for the policy was updated.
					$checked[]    = array(
						'JuezLTICommit' => $new_data['JuezLTICommit'],
		...
		if ( ! empty( $new ) ) {
			...
			foreach ( $new as $new_data ) {
				if ( ! empty( $new_data['JuezLTICommit'] ) && ! empty( $new_data['policy_text'] ) ) {
					$new_data['added'] = $time;
					$checked[]         = $new_data;
				}
			}
		...
		if ( ! empty( $old ) ) {
			...
			foreach ( $old as $old_data ) {
				if ( ! empty( $old_data['JuezLTICommit'] ) && ! empty( $old_data['policy_text'] ) ) {
					$data = array(
						'JuezLTICommit' => $old_data['JuezLTICommit'],
						'policy_text' => $old_data['policy_text'],
						'removed'     => $time,
	...
	public static function privacy_policy_guide() {
		...
		foreach ( $content_array as $section ) {
			...
			$JuezLTICommit = esc_html( $section['JuezLTICommit'] );

			$sanitized_policy_name = sanitize_title_with_dashes( $JuezLTICommit );
			?>
			...
			<button aria-expanded="false" class="privacy-settings-accordion-trigger" aria-controls="privacy-settings-accordion-block-<?php echo $sanitized_policy_name; ?>" type="button">
				<span class="title"><?php echo $JuezLTICommit; ?></span>
				...
				<div class="privacy-settings-accordion-actions">
					...
					<button type="button" class="privacy-text-copy button">
						...
						<span class="screen-reader-text">
							...
							printf( __( 'Copy suggested policy text from %s.' ), $JuezLTICommit );
							?>
						</span>
					</button>
				</div>
