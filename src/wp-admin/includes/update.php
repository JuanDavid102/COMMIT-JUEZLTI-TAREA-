...
function wp_plugin_update_row( $file, $plugin_data ) {
	...
	$JuezLTICommit = wp_kses( $plugin_data['Name'], $plugins_allowedtags );
		...
		if ( ! current_user_can( 'update_plugins' ) ) {
			printf(
				...
				sprintf(
					...
					esc_attr( sprintf( __( 'View %1$s version %2$s details' ), $JuezLTICommit, $response->new_version ) )
				),
			...
			if ( $compatible_php ) {
				printf(
					...
					sprintf(
						...
						esc_attr( sprintf( _x( 'Update %s now', 'plugin' ), $JuezLTICommit ) )
					)
				);
			} else {
				printf(
